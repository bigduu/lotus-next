...
test("desktop and tablet keep the shared browser frame while a person answers JavaScript dialogs", async ({ page }, testInfo) => {
  ...
  await panel.getByRole("tab", { name: "浏览器标签页 1：Example" }).click()
  ...
  await expect(page.getByRole("menuitem", { name: "浏览器" })).toBeDisabled()
  ...
  const workbenchTab = panel.getByRole("tab", { name: "浏览器标签页 1：Example" })
  ...
  const draft = browser.getByRole("textbox", { name: "网页地址" })
  await draft.fill("https://draft.test/keep")
  showDialog("alert")
  await expect(modal).toContainText("alert from page")
  await expect(draft).toHaveValue("https://draft.test/keep")
  await modal.getByRole("button", { name: "确定" }).click()
  await expect(modal).toHaveCount(0)
  await expect(draft).toHaveValue("https://draft.test/keep")
