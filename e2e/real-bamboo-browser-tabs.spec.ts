...
const exerciseSurface = async (
  ...
  try {
    ...
    await expect(pane.getByText("打开新网页")).toBeVisible()
    await address.fill(fixtureUrl("beta"))
    await pane.getByRole("button", { name: "打开", exact: true }).click()
    await expect(panel.getByRole("tab", { name: /浏览器标签页 2/ })).toHaveAttribute("aria-selected", "true")
    ...
    await panel.getByRole("tab", { name: "浏览器标签页 1：Alpha fixture" }).click()
    await expect(address).toHaveValue(fixtureUrl("alpha"))
    ...
    await expect(pane.getByLabel("DOM 快照", { exact: true })).toHaveCount(0)
    await expect(panel.getByRole("tab", { name: "浏览器标签页 3：Popup fixture" })).toHaveAttribute("aria-selected", "true")
    ...
    await panel.getByRole("button", { name: "关闭浏览器标签页 3：Popup fixture" }).click()
    await expect(panel.getByRole("tab", { name: "浏览器标签页 2：Beta fixture" })).toHaveAttribute("aria-selected", "true")
    await panel.getByRole("button", { name: "关闭浏览器标签页 2：Beta fixture" }).click()
    await expect(panel.getByRole("tab", { name: "浏览器标签页 1：Alpha fixture" })).toHaveAttribute("aria-selected", "true")
    ...
    await panel.getByRole("button", { name: "关闭浏览器标签页 1：Alpha fixture" }).click()
    await expect(panel.getByRole("tab", { name: /浏览器标签页/ })).toHaveCount(0)
    await expect(panel.getByText("还没有打开内容")).toBeVisible()
    const emptyResponse = await fetch(new URL(browserPath(sessionId), baseUrl), { signal: AbortSignal.timeout(5_000) })
    expect(emptyResponse.status).toBe(200)
    const empty = await emptyResponse.json() as { active_tab_id: string | null; tabs: unknown[] }
    expect(empty).toMatchObject({ active_tab_id: null, tabs: [] })
  } finally {
    await context.close()
  }
}
