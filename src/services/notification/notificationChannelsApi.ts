...
const encodeCredentialChange = (value: unknown, label: string): CredentialChange => {
  const raw = record(value, label)
  if (raw.action === "keep" || raw.action === "clear") {
    exactKeys(raw, ["action"], label)
    return { action: raw.action }
  }
  if (raw.action === "replace") {
    exactKeys(raw, ["action", "value"], label)
    const replacement = nonemptyString(raw.value, `${label}.value`)
    if (isMaskedSecret(replacement)) incompatible(`${label}.value must not be a credential mask`)
    return { action: "replace", value: replacement }
  }
  return incompatible(`${label}.action must be keep, replace, or clear`)
}

/** Own the wire shape here: callers need not rely on a particular editor's validation. */
const encodeMutationData = (value: unknown): NotificationMutationData => {
  const raw = record(value, "Notification mutation")
  exactKeys(raw, ["desktop", "ntfy", "bark"], "Notification mutation")
  const desktop = record(raw.desktop, "Notification mutation.desktop")
  exactKeys(desktop, ["enabled"], "Notification mutation.desktop")
  if (desktop.enabled !== null && typeof desktop.enabled !== "boolean") {
    incompatible("Notification mutation.desktop.enabled must be a boolean or null")
  }
  const ntfy = record(raw.ntfy, "Notification mutation.ntfy")
  exactKeys(ntfy, ["enabled", "base_url", "topic", "credential_change"], "Notification mutation.ntfy")
  if (typeof ntfy.enabled !== "boolean") {
    incompatible("Notification mutation.ntfy.enabled must be a boolean")
  }
  const bark = record(raw.bark, "Notification mutation.bark")
  exactKeys(bark, ["enabled", "base_url", "credential_change"], "Notification mutation.bark")
  if (typeof bark.enabled !== "boolean") {
    incompatible("Notification mutation.bark.enabled must be a boolean")
  }
  return {
    desktop: { enabled: desktop.enabled as boolean | null },
    ntfy: {
      enabled: ntfy.enabled as boolean,
      base_url: secretFreeHttpUrl(ntfy.base_url, "Notification mutation.ntfy.base_url"),
      topic: stringValue(ntfy.topic, "Notification mutation.ntfy.topic"),
      credential_change: encodeCredentialChange(
        ntfy.credential_change,
        "Notification mutation.ntfy.credential_change",
      ),
    },
    bark: {
      enabled: bark.enabled as boolean,
      base_url: secretFreeHttpUrl(bark.base_url, "Notification mutation.bark.base_url"),
      credential_change: encodeCredentialChange(
        bark.credential_change,
        "Notification mutation.bark.credential_change",
      ),
    },
  }
}
...
export const putNotificationChannelsConfig = async (
  before: NotificationConfigEnvelope,
  mutation: NotificationMutationData,
): Promise<NotificationConfigEnvelope> => {
  const data = encodeMutationData(mutation)
