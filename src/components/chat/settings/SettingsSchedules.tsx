...
export function SettingsSchedules() {
  ...
  const add = async () => {
    ...
    await agentClient
      .createSchedule({
        name: name.trim(),
        trigger: { type: "cron", expression: cron.trim() } as unknown as ScheduleTrigger,
        enabled: true,
    ...
    <div className="space-y-3">
      ...
      {adding ? (
        <div className="space-y-2.5 rounded-lg border bg-muted/30 p-3">
          ...
          <textarea className={cn(input, "min-h-16 resize-y")} placeholder="任务内容" value={task} onChange={(e) => setTask(e.target.value)} />
          <div className="flex justify-end gap-2">
            <Button size="sm" variant="secondary" onClick={() => setAdding(false)}>
              取消
            </Button>
            <Button size="sm" onClick={add} disabled={!name.trim() || !task.trim()}>
              添加
            </Button>
          </div>
        </div>
